    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (allQuestions.length < 10) {
                console.warn('Numero domande valide inferiore a 10, il quiz userà tutte le disponibili:', allQuestions.length);
            }
            ...
            if (i === retries - 1) {
                ...
                errorDiv.innerHTML = `
                    ...
                    <button class="btn error-retry-btn">🔄 Ricarica Pagina</button>
                `;
...
function showQuestion(question) {
    ...
    question.answers.forEach(answer => {
        ...
        const span = document.createElement('span');
        span.textContent = sanitizeText(answer.text);
...
function selectAnswer(e) {
    ...
    userAnswers.push({
        ...
        userAnswer: selectedButton.querySelector('span').textContent,
